...
Learn how to test BetterService services and workflows.
...
# 4. Error handling - Errors are raised appropriately
# 5. Side effects - Database changes, etc.
```
...
Verify value constraints are enforced.
...
test "validates price must be positive" do
  ...
  assert error.context[:validation_errors].key?(:price)
  assert_includes error.context[:validation_errors][:price].to_s, "greater than"
end
...
test "allows owner to update own product" do
  owner = users(:seller)
  product = products(:seller_product)  # Belongs to seller
...
test "denies non-owner from updating product" do
  ...
  product = products(:seller_product)  # Belongs to different user
...
Verify not found errors are raised.
