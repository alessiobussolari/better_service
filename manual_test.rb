...
ActiveRecord::Base.transaction do
  ...
  test("create service validates params") do
    invalid_result = Product::CreateService.new(user, params: {
      name: "",
      price: -10
    }).call

    !invalid_result[:success] && invalid_result[:errors].present?
  end
  ...
  test("show service fails with invalid id") do
    fail_result = Product::ShowService.new(user, params: { id: 99999 }).call
  ...
  test("update service validates changes") do
    invalid_update = Product::UpdateService.new(user, params: {
      id: @product_id,
      price: -50
    }).call

    !invalid_update[:success]
  end
